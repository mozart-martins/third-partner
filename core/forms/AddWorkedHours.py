...
class AddWorkedHours(forms.ModelForm):
    class Meta:
        ...
        labels = {'worker': 'Trabalhador', 'date': 'Data',
                  'worked_hours': 'Horas Trabalhadas', 'description': 'Descrição'}
        widgets = {
            'worker': forms.TextInput(attrs={'class': 'form-control', 'type': 'hidden'}),
            'date': forms.DateInput(attrs={'class': 'form-control'}),
            'worked_hours': forms.TimeInput(format='%H:%M' attrs={'class': 'form-control', 'type': 'time'}),
